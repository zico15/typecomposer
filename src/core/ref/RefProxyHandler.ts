...
export function createProxyRef<T extends object>(
    ...
    const registry: FinalizationRegistry<any> = new FinalizationRegistry(
        (target: WeakRef<any>) => {
            _subscribers = _subscribers.filter(
                (subscriber) => (subscriber.target as any) != target,
            );
        },
    );
    ...
    function setValueToSubscriber(
        ...
        try {
            ...
            if (target) {
                ...
                if (
                    subscriber.refPropertyKey == undefined ||
                    prop == undefined ||
                    prop == subscriber.refPropertyKey
                )
    ...
    proxy["subscriber"] = function (
        target: {},
        propertyKey: string | symbol,
        refPropertyKey: string | symbol = undefined, writeOnly: boolean = false,
    ) {
        if (writeOnly) {
        ...
        let value =
            refPropertyKey == undefined ? proxy["value"] : proxy[refPropertyKey];
        if (value instanceof RefString) value = value.toString();
